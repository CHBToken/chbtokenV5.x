...
{
    resize(850+95, 550);
    setWindowTitle(tr("CHBToken") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a CHBToken address"));
    ...
    aboutAction = new QAction(tr("&About CHBToken"), this);
    aboutAction->setToolTip(tr("Show information about CHBToken"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for CHBToken"));
	...
	uploadFileAction = new QAction(tr("Upload File and Sign"), this);
    purchaseChbtAction = new QAction(tr("Purchase CHBToken"), this);
	signFileAction = new QAction(tr("Sign File only"), this);
	signingHelpAction = new QAction(tr("Signing Help"), this);
	viewDocumentsAction = new QAction(tr("View Documents"), this);
	chbtPayGateVersionAction = new QAction(tr("Paygate Version"), this);
	...
	connect(uploadFileAction, SIGNAL(triggered()), this, SLOT(uploadFileClicked()));
    connect(purchaseChbtAction, SIGNAL(triggered()), this, SLOT(purchaseChbtClicked()));
	connect(signFileAction, SIGNAL(triggered()), this, SLOT(signFileClicked()));
	connect(signingHelpAction, SIGNAL(triggered()), this, SLOT(signingHelpClicked()));
	connect(viewDocumentsAction, SIGNAL(triggered()), this, SLOT(viewDocumentsClicked()));
	connect(chbtPayGateVersionAction, SIGNAL(triggered()), this, SLOT(chbtPayGateVersionClicked()));
}
...
void BitcoinGUI::createMenuBar()
{
    ...
    int chbtgateEnabled = -1;
    //  Check for existence of DLSPaymentProcessor
    QString paygate = "./CHBTPaygate.exe";
    QFileInfo check_file(paygate);
    ...
    if((check_file.exists() && check_file.isFile()))
    {
        ...
        chbtgateEnabled = myProcess->exitCode();
        ...
        {
            QMenu* uploadfile = appMenuBar->addMenu(tr("File Signing and Storage"));
            uploadfile->addAction(uploadFileAction);
            uploadfile->addAction(signFileAction);
            uploadfile->addSeparator();
            uploadfile->addAction(viewDocumentsAction);

            QMenu* buychbtoken = appMenuBar->addMenu(tr("Purchase"));
            buychbtoken->addAction(purchaseChbtAction);
        }
    }
	...
	help->addSeparator();
	help->addAction(signingHelpAction);
    if (chbtgateEnabled == 0)
    {
        help->addAction(chbtPayGateVersionAction);
    }
}
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("CHBToken client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("CHBToken client"));
...
void BitcoinGUI::uploadFileClicked()
{
    UploadFileDialog dlg;
    dlg.setModel(walletModel);
    dlg.exec();
}

void BitcoinGUI::signFileClicked()
{
    SignFileDialog dlg;
    dlg.setModel(walletModel);
    dlg.exec();
...
void BitcoinGUI::purchaseChbtClicked()
{
    PurchaseDialog dlg;
    dlg.setModel(walletModel, clientModel);
    dlg.exec();
}

void BitcoinGUI::signingHelpClicked()
{
    SigningHelpDialog dlg;
   // dlg.setModel(clientModel);
    dlg.exec();
}

void BitcoinGUI::viewDocumentsClicked()
{
    ViewDocumentsDialog dlg;
   // dlg.setModel(clientModel);
    dlg.exec();
}

void BitcoinGUI::chbtPayGateVersionClicked()
{
    CHBTPayGateVersionDialog dlg;
   // dlg.setModel(clientModel);
    dlg.exec();
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to CHBToken network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid CHBToken address or malformed URI parameters."));
    }
